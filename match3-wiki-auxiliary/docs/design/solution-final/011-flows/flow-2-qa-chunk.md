...
```mermaid
sequenceDiagram
    participant 客户端 as 客户端<br/>(Next.js QAChat)
    participant API as API 层<br/>(FastAPI /qa/ask)
    ...
    Note over SVC,Router: 阶段 1 — 路由分类
    ...
    RAG->>RAG: RRF 融合三路结果 → ranked id list

    Note over RAG,PG: 阶段 3 — 从 PostgreSQL 取 content
    RAG->>PG: find_by_ids(ranked_ids[:top_k])
    PG-->>RAG: [TextChunk{id, content, ...}]
    RAG-->>SVC: candidates[{id, content, score}]
    deactivate RAG

    Note over SVC,Reranker: 阶段 4 — 重排序
    ...
    Note over SVC,LLM: 阶段 5 — LLM 流式生成
...
| 4 | Router → LLM | 路由器调用 `rt.llm.complete()` 以少样本 prompt 对问题分类，返回路径标签字符串（`hybrid-search` / `wiki-lookup` / `doc-navigate`）。分类 prompt 包含三条路径的特征描述和示例，保证分类准确。 |
