...
sequenceDiagram
    ...
    Note over 客户端,Router: 阶段 B — 路由分类（与流程 2 共用入口）
    客户端->>API: GET /api/v1/qa/ask?query=...[SSE]
    ...
    Note over SVC,PageIndex: 阶段 C — 目录树导航
    SVC->>PG: 查询 workspace 内所有 pageindex_doc_id IS NOT NULL 的文件
    PG-->>SVC: [{raw_file_id, pageindex_doc_id}, ...]

    loop 每个 PageIndex 文档
        SVC->>PageIndex: get_tree(doc_id) → tree
        PageIndex-->>SVC: 层级目录树节点列表
    ...
    SVC->>SVC: 合并相关段落 → context

    Note over SVC,LLM: 阶段 D — LLM 流式生成（与流程 2 相同）
...
| 8 | QAService → LLM（流式） | 与流程 2 的阶段 5 相同：将 context 和用户问题构造 prompt，调用 `rt.llm.stream()` 获取流式 token。 |
