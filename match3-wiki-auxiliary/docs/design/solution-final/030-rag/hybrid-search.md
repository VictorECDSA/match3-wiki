# hybrid-search：可配置 5 阶段检索流水线

## 1. 设计动机

原始设计把 16 种 RAG 方法平铺并列，但这些方法之间存在大量重叠与嵌套关系：
...
切块之前先把各种格式的原始文档统一转成 Markdown，原因是：Markdown 的标题层级天然提供切块边界，可以做到**零 LLM 调用的结构化切块**；转换后剔除了 PDF/DOCX 的格式噪声，chunk 质量更高。
...
**实现方式：** 把步骤二产生的 chunks 当作 parent，对每个 parent 再用 fixed_size 切出更小的 child。向量索引只存 child，每个 child 携带 `parent_id`。检索时命中 child，返回时取对应 parent（parent_splitter: 1500 字，child_splitter: 300 字）。
...
### 7.2 BM25（Elasticsearch）：倒排索引 + filter 前置
...
当前版本的 Multi-Agent 实现（按领域拆分检索）详见 `030-rag/multi-agent.md`。
...
路由器 `AdaptiveRAGRouter` 输出的 `complexity` 字符串通过 `PROFILE_MAP` 映射到对应 Profile，详见 `030-rag/overview.md`。
...
| Multi-Query | `multi_query=True`，见第 8.1 节 |
| HyDE | `hyde=True`，见第 8.2 节 |
...
| Multi-Agent RAG | 见 `030-rag/multi-agent.md` |
