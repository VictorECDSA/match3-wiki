...
# Broker/backend URLs come from .env (sensitive config), read via Env at import time.
# Non-sensitive task config (retry counts, timeouts) comes from config.yaml, read via get_runtime().
...
celery_app.conf.update(
    ...
    worker_prefetch_multiplier=1, # each worker slot processes only one task at a time
    task_routes={
...
from __future__ import annotations
from app.runtime.runtime import Match3Runtime, build_runtime
...
```sql
-- Find files stuck in an intermediate state for more than 15 minutes
SELECT f_raw_file_id, f_status, f_updated_at
FROM t_raw_files
WHERE f_status IN ('PROCESSING')
  AND f_updated_at < NOW() - INTERVAL '15 minutes';
-- Re-enqueue the corresponding Celery task for each returned row
```
...
```bash
# ingest + embed worker (high throughput)
...
# graph worker (IO-bound, lower concurrency is sufficient)
celery -A app.workers.celery_app worker \
  --queues=graph \
  --concurrency=2 \
  --loglevel=info \
...
# compile worker (LLM-intensive, time-limited)
celery -A app.workers.celery_app worker \
  --queues=compile \
  --concurrency=2 \
  --loglevel=info \
